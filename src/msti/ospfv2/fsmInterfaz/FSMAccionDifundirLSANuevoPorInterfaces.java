...
public class FSMAccionDifundirLSANuevoPorInterfaces implements FSMAccion {
	/** La instancia singleton */
	protected static FSMAccionDifundirLSANuevoPorInterfaces _instancia = new FSMAccionDifundirLSANuevoPorInterfaces();
	...
	protected FSMAccionDifundirLSANuevoPorInterfaces() {
	}
	...
	public void difundirLinkStateUpdatePorInterfaz(FSMMaquinaEstadosOSPFv2Interfaz interfaz, IMensajeOSPFv2LSA lsaAEnviar){
		...
		estadoInterfaz.getId().equals(FSMEstadoOSPFv2Interfaz.FSMIdEstadoOSPFv2Interfaz.DR)){
			escritura.setDireccionDestino(new InetSocketAddress(contextoI.getConfiguracion().allSPFRouters, 0)); //direccion AllSPFRouters
		}else{
			//Sino, solo a los DR y BDR
			escritura.setDireccionDestino(new InetSocketAddress(contextoI.getConfiguracion().allDRouters, 0)); //direccion AllDRouters
		}
	...
	public void execute(FSMContexto contexto, Object o) {
		...
		FSMContextoOSPFv2Interfaz contextoIOrigen = (FSMContextoOSPFv2Interfaz) contexto;
		FSMContextoOSPFv2Interfaz contextoI;
		long claveLSA = contextoIOrigen.getConfiguracion().claveLSA(lsaADifundir.getHeader().getLSType(), lsaADifundir.getHeader().getLinkStateID());
		
		FSMEstadoOSPFv2Vecino estadoVecinoIterado;
		...
		for(FSMMaquinaEstadosOSPFv2Interfaz interfaz: contextoIOrigen.getConfiguracion().listaFSMInterfaz){
			...
			for(FSMMaquinaEstadosOSPFv2Vecino vecino: contextoI.getListOfNeighbouringRouters().values()){
				...
				estadoVecinoIterado = (FSMEstadoOSPFv2Vecino) vecino.getEstadoActivo();
				//1.a. Si el vecino está en estado menor a Exchange, no participa en el proceso				
				if(estadoVecinoIterado.getId().equals(FSMEstadoOSPFv2Vecino.FSMIdEstadoOSPFv2Vecino.EXCHANGE) ||
					estadoVecinoIterado.getId().equals(FSMEstadoOSPFv2Vecino.FSMIdEstadoOSPFv2Vecino.LOADING) ||
					estadoVecinoIterado.getId().equals(FSMEstadoOSPFv2Vecino.FSMIdEstadoOSPFv2Vecino.FULL)){
					...
					if(estadoVecinoIterado.getId().equals(FSMEstadoOSPFv2Vecino.FSMIdEstadoOSPFv2Vecino.EXCHANGE) ||
						...
						if(contextoVIterado.getLinkStateRequestList().containsKey(claveLSA)){
							...
							if(1==vecino.determinarQueLSAEsMasReciente(contextoVIterado, lsaADifundir.getHeader(),
							...
							}else if(0==vecino.determinarQueLSAEsMasReciente(contextoVIterado, lsaADifundir.getHeader(),
							contextoVIterado.getLinkStateRequestList().get(claveLSA))){
								contextoVIterado.getLinkStateRequestList().remove(claveLSA);
					...
					//pasos 1.c y 1.d
					if(pasos1Cy1D){
						//1.c. Si el LSA fue recibido por este vecino, pasar al siguiente (imposible ya que este es creado por este router)	
