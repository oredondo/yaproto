...
public class MensajeOSPFv2LSANetworkLinksAdvertisements extends MensajeOSPFv2LSA implements IMensaje, IMensajeOSPFv2LSANetworkLinksAdvertisements {
	...
	*/
	...
	protected boolean hasAttachedRouters=false;
	
	protected int networkMask;
	protected List<Integer> attachedRouters;
	
	protected MensajeOSPFv2LSANetworkLinksAdvertisements() {
		super();
	}
	...
	public void writeToOutputStream(OutputStream output) throws IOException {
		...
		//escribe routerLinks
		if (hasAttachedRouters){
			for (Integer attachedRouter: attachedRouters)
				dos.writeInt(attachedRouter);				
		...
		short packetLengthLSANetwork = (short) getLongitudSerializado();
		mensajeSerializadoLSA[18] =  (byte)((packetLengthLSANetwork >> 8) & 0xff);
		mensajeSerializadoLSA[19] = (byte)(packetLengthLSANetwork & 0xff);
		//header.packetLength = packetLengthLSANetwork;
		
		//calcular checksum
		//checksum está a 0, calcularlo, rellenarlo (en el array mensaje Serializado)
		short checksumLSANetwork = (short) ChecksumOSPFv2.calcularChecksumLSA(mensajeSerializadoLSA);
		mensajeSerializadoLSA[16] =  (byte)((checksumLSANetwork >> 8) & 0xff);
		mensajeSerializadoLSA[17] = (byte)(checksumLSANetwork & 0xff);
		//header.lSChecksum=checksumLSANetwork;
	...
	public int getLongitudSerializado() {
		...
		return super.getLongitudSerializado() + 4 + 4*attachedRouters.size();
	}
	 ...
	 */
	public static class Builder extends MensajeOSPFv2LSA.Builder implements IMensajeBuilder, IMensajeOSPFv2LSANetworkLinksAdvertisements, IMensajeOSPFv2LSANetworkLinksAdvertisements.Build {
		
		private Builder() {
			//super();
			mensaje = new MensajeOSPFv2LSANetworkLinksAdvertisements();
		}

		protected MensajeOSPFv2LSANetworkLinksAdvertisements getMensaje() {
			return (MensajeOSPFv2LSANetworkLinksAdvertisements)this.mensaje;
		}
		...
		@Override
		public MensajeOSPFv2LSANetworkLinksAdvertisements build() {
		...
		public static MensajeOSPFv2LSANetworkLinksAdvertisements getDefaultInstanceforType() {
			MensajeOSPFv2LSANetworkLinksAdvertisements mensaje = new MensajeOSPFv2LSANetworkLinksAdvertisements();
		...
		public Builder mezclarDesde(InputStream inputStream, byte[] mensajeSerializadoLSA) throws IOException {	
			...
			if(ChecksumOSPFv2.verificarChecksumLSA(mensajeSerializadoLSA)){
				...
				//attachedRouters
				//Sededucen el número de attachedRouters del length.
				int nAttachedRouters = (this.getHeader().getLength()-24)/4;
				
				List<Integer> attachedRouters = new ArrayList<Integer>();
				this.setAttachedRouters(attachedRouters);
				for(short i=0;i<nAttachedRouters;i++){
					this.addAttachedRouters(dis.readInt());		
				}
		...
		public IMensajeBuilder mezclarDesde(IMensaje mensajeOrigen) {
			...
			else if (mensaje instanceof IMensajeOSPFv2LSANetworkLinksAdvertisements){
				IMensajeOSPFv2LSANetworkLinksAdvertisements _mensajeOrigen = (IMensajeOSPFv2LSANetworkLinksAdvertisements) mensajeOrigen;

				if (_mensajeOrigen.hasNetworkMask())
					this.setNetworkMask(_mensajeOrigen.getNetworkMask());

				if (_mensajeOrigen.hasAttachedRouters())
					this.setAttachedRouters(_mensajeOrigen.getAttachedRouters());
				
			}
			else
				throw new IllegalArgumentException("IMensajeOSPFv2LSANetworkLinksAdvertisements::mezclarDesde(IMensaje): objeto recibido no es de clase IMensajeOSPFv2LSANetworkLinksAdvertisements");
		...
		public boolean estaCompleto() {
			return ((getMensaje().hasNetworkMask) && (getMensaje().hasAttachedRouters)); 
		}
		...
		public List<Integer> getAttachedRouters() {
			// TODO Auto-generated method stub
			return getMensaje().attachedRouters;
		}
		...
		public Builder setAttachedRouters(List<Integer> attachedRouters) {
			getMensaje().attachedRouters =attachedRouters;
			getMensaje().hasAttachedRouters = true;
			return this;
		}

		
		public Builder removeAttachedRouters() {
			getMensaje().attachedRouters = null;
			getMensaje().hasAttachedRouters = false;
			return this;
		}
		
		@Override
		public Builder addAttachedRouters(Integer attachedRouter) {
			if (getMensaje().attachedRouters == null)
				getMensaje().attachedRouters = new ArrayList<Integer>();
			getMensaje().attachedRouters.add(attachedRouter);
			getMensaje().hasAttachedRouters = true;
			return this;
		}
		...
		public boolean hasAttachedRouters() {
			return getMensaje().hasAttachedRouters;
		}
	...
	public boolean hasAttachedRouters() {
		return this.hasAttachedRouters;
	}
	...
	public String toString() {
		...
		if (hasAttachedRouters)
			sb.append(",AttachedRouters ");
			for (Integer attachedRouter: attachedRouters)
				sb.append(Integer.toString(attachedRouter) + " ");
