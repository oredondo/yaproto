...
public class FSMEstadoOSPFv2VecinoLoading extends FSMEstadoOSPFv2Vecino {
	static {
		_instancia = new FSMEstadoOSPFv2VecinoLoading(FSMIdEstadoOSPFv2Vecino.LOADING);
	}

	protected FSMEstadoOSPFv2VecinoLoading(FSMIdEstado id) {		
		super(id);
	}
	...
	public FSMEstado procesarEventoExchangeDone(
		...
		//Comprobar si la LinkStateRequestList esta vac�a y si se ha terminado co los DatabaseDescriptionPackets, en ese caso LOADINGDONE
		if((contextoV.getLinkStateRequestList().size() == 0) && contextoV.isAllDatabaseDescriptionPacketSent()){
			guarda="LoadingDone";
		}else{
			guarda="ExchangeDone";
		}
	...
	public FSMEstado procesarEventoAdjOK(
		...
		System.out.println("FSMEstadoOSPFv2VecinoLoading:eventoAdjOK(): guarda= " + guarda);
