...
public class EscritorRawSocketNetlink extends Escritor {

	/* tipo RawSocket no deriva de DatagramSocket */
	...
	public EscritorRawSocketNetlink() {
		super();
	}
		...
		if (solicitadoOutputStream) {
 		...
 		try {
			System.out.println("<---- Env�o: " + bufer.length + " octetos. Destino:" + escritura.getDireccionDestino().toString());
 			// Env�o del datagrama
			// TODO: Temporal: ignoral direccionDestino herencia de SocketAddress y genera una interna
			NetlinkAddress direccion = socket.new NetlinkAddress();
			direccion.pid = 0;
			direccion.grupos = 0;
