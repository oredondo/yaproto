...
public class AceptadorDatagrama extends Aceptador {
	...
	public void run() {
		// Verifica bind()
		if (this.socket == null) {
			throw new IllegalStateException("Ejecutado run() en AceptadorDatagrama, sin realizar bind()");
		}
		// Verifica filtros
		if (this.getCadenaFiltros().size() == 0)
			//TODO: quiz� cadenaFiltros podr�a a�adir siempre el filtro final (notificador)
			throw new IllegalStateException("Ejecutado run() en AceptadorDatagrama, con cadenafiltros vac�a");
