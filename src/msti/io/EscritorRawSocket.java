...
public class EscritorRawSocket extends Escritor {
	...
	public void escribir(Sesion sesion, Escritura escritura) {
		...
		if (solicitadoOutputStream) {
 		...
 		try {
			System.out.println("<---- Env�o: " + bufer.length + " octetos. Destino:" + escritura.getDireccionDestino().toString());
 			// Env�o del datagrama
			// TODO: Temporal: obtiene la direccionDestino parte de SocketAddress, por no hacer versión de Escritura con destino InetAddress y dejarla genérica
			InetAddress direccion = ((InetSocketAddress)escritura.getDireccionDestino()).getAddress();
			socket.write(direccion, bufer);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
