...
public class MensajeNetlinkRouteAttributePrefsrc extends MensajeNetlinkRouteAttribute implements IMensaje, IMensajeNetlinkRouteAttributePrefsrc {
	...
	protected boolean hasSrc = false;
	
	protected MensajeNetlinkRouteAttributePrefsrc() {
		super();		
	}
	...
	public void writeToOutputStream(OutputStream output) throws IOException {
		...
		/* src */
		byte[] bufer = this.getSrc().getAddress(); 
	...
	protected int getLongitudMensaje() {
		...
		// Datos
		total += this.getSrc().getAddress().length;  
	 ...
	 */
	public static class Builder extends MensajeNetlinkRouteAttribute.Builder implements IMensajeBuilder, IMensajeNetlinkRouteAttributePrefsrc, IMensajeNetlinkRouteAttributePrefsrc.Build {
		
		private Builder() {
			_builder = this;
			mensaje = new MensajeNetlinkRouteAttributePrefsrc(); //inicializa el mensaje heredado con el tipo adecuado
		}

		/* Sobreescribe para cambiar el tipo devuelto y usarlo tipado donde se requiera */
		@Override
		protected MensajeNetlinkRouteAttributePrefsrc getMensaje() {
			return (MensajeNetlinkRouteAttributePrefsrc)this.mensaje;
		}
		...
		@Override
		public MensajeNetlinkRouteAttributePrefsrc build() {
		...
		public static MensajeNetlinkRouteAttributePrefsrc getDefaultInstanceforType() {
			// TODO: Petici�n de tabla de rutas puede ser el mensaje por defecto
			MensajeNetlinkRouteAttributePrefsrc mensaje = new MensajeNetlinkRouteAttributePrefsrc();
			mensaje.estaConstruido = true;
			return mensaje;
		}
		...
		public Builder mezclarDesde(InputStream inputStream) throws IOException {
			...
			dis.read(bufer);
			this.setSrc(InetAddress.getByAddress(bufer)); // siempre en orden de red			
			leidos += bufer.length;
		...
		public Builder mezclarDesde(IMensaje mensajeOrigen) {
			...
			if (mensaje instanceof IMensajeNetlinkRouteAttribute) {
			...
			else if (mensaje instanceof IMensajeNetlinkRouteAttributePrefsrc) {
				...
				MensajeNetlinkRouteAttributePrefsrc _mensajeOrigen = (MensajeNetlinkRouteAttributePrefsrc)mensajeOrigen;
				if (_mensajeOrigen.hasSrc())
					this.setSrc(_mensajeOrigen.getSrc());
			}
			else 
		...
		/* IMensajeNetlinkRouteAttributeSrc */

		@Override
		public Builder setSrc(InetAddress src) {
			getMensaje().src = src;
			getMensaje().hasSrc = true;			
			return this;
		}

		@Override
		public InetAddress getSrc() {
			return getMensaje().src;
		}

		@Override
		public boolean hasSrc() {
			return getMensaje().hasSrc;
		}
	...
	public String toString() {
		...
		sb.append(" MensajeNetlinkRouteAttributeSrc");
		...
		sb.append("\n Datos:");
		sb.append("  src="); 			sb.append(this.getSrc());
